...
const user = getUser();  // <-- Get the current user from token
// Fetch doctors from composable
const { doctors, loading, error, fetchAllDoctors } = useDoctors();
const doctor = ref<{ full_name: string; department: string } | null>(null);

onMounted(async () => {
  const id = route.query.receiver_id;
  if (id) {
    receiverId.value = parseInt(id as string);
    await fetchAllDoctors(); // optional
    doctor.value = doctors.value.find(doc => doc.id === receiverId.value) || null;
    await fetchChatHistory(receiverId.value); // <- Fetch messages first
    startWebSocket(receiverId.value);        // <- Then connect WebSocket
  }
});


// Start WebSocket connection
const startWebSocket = (receiverId: number) => {
  ...
  if (!messages.value.some(msg => msg.timestamp === data.timestamp)) {
    messages.value.push({
      ...data,
  ...
  setTimeout(() => startWebSocket(receiverId), 3000);
};
...
const sendMessage = () => {
  if (socket.value && newMessage.value.trim()) {
    console.log('Sending:', newMessage.value); // ✅ debug log
...
const fetchChatHistory = async (receiverId: number) => {
  ...
  try {
    const res = await fetch(`${apiBase}/patients/chat/history?receiver_id=${receiverId}`, {
      headers: {
    ...
    messages.value = history.map((msg: { sender_id: number; }) => ({
      ...msg,
    ...
    <div class="h-[1080px]">
      <div class="flex h-full">
        ...
        <main class="flex-1 bg-emerald-50 p-8 overflow-y-auto">
          <div class="bg-white rounded-xl shadow-lg h-full p-6 flex flex-col">
            <!-- Header -->
            ...
            <!-- Messages -->
            <div class="flex-1 bg-gray-50 rounded-xl p-6 mb-6 overflow-y-auto" ref="messageContainer">
              ...
              <div v-else>
                <div v-for="(msg, index) in messages" :key="index" class="mb-4">
                  <div
                    :class="[
                      'p-3 rounded-xl max-w-[70%] text-sm',
                      msg.is_self ? 'ml-auto bg-emerald-600 text-white' : 'bg-white text-gray-800'
                    ]"
                  >
                    {{ msg.message }}
                    <div class="mt-1 text-[10px] text-gray-400 text-right">{{ msg.timestamp }}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center space-x-4">
              <div class="flex-1">
                <input
                  ...
                  class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200"
                />
              </div>
              <button
                @click="sendMessage"
                class="bg-emerald-600 text-white p-3 rounded-xl hover:bg-emerald-700 transition-all duration-300 transform hover:scale-105"
              >
